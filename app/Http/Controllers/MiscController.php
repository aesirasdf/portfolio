    ...
    public function fakeTerminal(Request $request)
    {
        ...
        do {
            $result = $client->chat()->create([
                "model" => "gpt-4o-mini",
                "messages" => [
                    ["role" => "system", "content" => "remove the beginning and end of your message. make it in json format with message key. remove all [your name] make it generic. you are a linux terminal, and you should respond like one. the user will input a linux command (fedora) and you have to say appropriate message about my portfolio (Jeco Renz Concepcion, Systems Developer). the user is a guest that is viewing my portfolio. don't allow it to do anything bad to my portfolio only read access only. add unrecognized command if the command is not a valid linux command"],
